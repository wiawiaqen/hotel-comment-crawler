            ...
            try:
                with open(f"./Data/{province}/{file}", "r", encoding="utf-8") as f:
                    ...
                    for object in data:
                        ...
                        review_positive_comments.append(object['review_positive_comments'])
                        review_negative_comments.append(object['review_negative_comments'])
                        group_name.append(object['groupName'])
                        type_name.append(object['typeName'])
                        review_province.append(province)
            except Exception as e:
                print(e)
        data = {'reviewer_name': reviewer_name, 'reviewer_country': reviewer_country, 'review_date': review_date, 'stay_length': stay_length,'rating': rating, 'rating_text': rating_text, 'review_title': review_title, 'review_comments': review_comments, 'review_positive_comments': review_positive_comments, 'review_negative_comments': review_negative_comments, 'group_name':group_name, 'type_name':type_name, 'province': province}
        df = pd.DataFrame(data)
        df.to_excel(f"./Data/Agoda_{province}.xlsx", index=False, encoding='utf-8', engine='xlsxwriter')
