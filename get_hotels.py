...
async def get_data(
    session: aiohttp.ClientSession, province_ID: int, name: str, offset: int
):
    """
    ...
    Args:
        ...
        province_ID (int): ID of province
        name (str): name of province
        offset (int): offset of page
    """
    ...
    headers = {
        ...
        "referer": "https://www.agoda.com/vi-vn/search?city=16440",
        ...
        "variables": {
            "CitySearchRequest": {
                ...
                "searchRequest": {
                    "searchCriteria": {
                        ...
                        "checkInDate": f"{oneweek_later}T07:47:02.895Z",
                        "localCheckInDate": f"{oneweek_later}",
                        "los": 1,
                        "rooms": 1,
                        "adults": 1,
                        "children": 0,
                    ...
                    "searchContext": {
                        ...
                        "pageTypeId": 103,
                        "whiteLabelKey": None,
                        "ipAddress": "42.115.115.121",
                        "endpointSearchType": "CitySearch",
                        "trackSteps": None,
                        "searchId": "ec7d1165-a723-4f1e-a4f3-1d342e1cfccb",
                    },
                    "matrix": None,
                    "matrixGroup": [
            ...
            "ContentSummaryRequest": {
                "context": {
                    ...
                    "apo": False,
                    "searchCriteria": {"cityId": int(province_ID)},
                    "platform": {"id": 1},
                    "storeFrontId": 3,
                    "cid": "-1",
                    "occupancy": {
...
def extract_data(response: dict, offset: int, name: str):
    ...
    """
    try:
        usable_data = response["data"]["citySearch"]["properties"]
    except:
        try:
            usable_data = response["data"]["citySearch"]["highlyRatedAgodaHomes"]
        except:
            return
    final_data = []
    for object in usable_data:
        new = {}
        new["id"] = object["propertyId"]  # thu thập id
        new["hotel_name"] = object["content"]["informationSummary"][
            "displayName"
        ]  # thu thập tên khách sạn
        new["display_location"] = object["content"]["informationSummary"]["address"][
            "area"
        ][
            "name"
        ]  # thu thập địa điểm
        new["city"] = object["content"]["informationSummary"]["address"]["city"][
            "name"
        ]  # thu thập thành phố
        try:
            new["score"] = object["content"]["reviews"]["cumulative"][
                "score"
            ]  # thu thập điểm đánh giá
        except:
            new["score"] = None  # điểm đánh giá đôi khi không hiển thị
        try:
            new["total_reviews"] = object["content"]["reviews"]["cumulative"][
                "reviewCount"
            ]  # thu thập tổng số đánh giá
        except:
            new["total_reviews"] = 0  # tổng số đánh giá đôi khi không hiển thị
        final_data.append(new)
    try:
        with open(
            f"./Hotel/{name}/data_page_{int(offset/45)+1}.json", "w", encoding="utf-8"
        ) as file:
            json.dump(final_data, file, indent=4, ensure_ascii=False)
            print(f"{name} - {int(offset/45)+1}")
    except Exception as e:
        print(e)


async def get_all_data(
    session: aiohttp.ClientSession, province_id: int, name: str, range: int
) -> list[asyncio.Task]:
    """
    ...
    tasks = []
    for i in range:
        tasks.append(asyncio.create_task(get_data(session, province_id, name, i)))
...
async def fetch_all(session: aiohttp.ClientSession) -> list[asyncio.Task]:
    """
    fetch all data from province file
    ...
    for object in file:
        province = object["name"]
        if not os.path.exists(f"./Hotel/{province}"):
            os.makedirs(f"./Hotel/{province}")
            local_dir = f"./Hotel/{province}"
        province_ID = object["id"]
        name = object["name"]
        total = object["total"]
        x = range(0, total, 45)
        tasks.append(asyncio.create_task(get_all_data(session, province_ID, name, x)))
    res = await asyncio.gather(*tasks)
    return res
